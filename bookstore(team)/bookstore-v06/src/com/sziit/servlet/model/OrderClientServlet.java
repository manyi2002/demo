...
import com.sziit.Service.Impl.UserServiceImpl;
import com.sziit.Service.OrderService;
import com.sziit.Service.UserService;
import com.sziit.Utils.PageSupport;
    ...
    public void orderManage(HttpServletRequest request, HttpServletResponse response) throws IOException {
        try {
            ...
            if(user1!=null) {
                ...
                int totalPageCount = pageSupport.getTotalPageCount();
                //将数据记录的总条数信息传递给前端
                request.setAttribute("toCount",orderCount);
                //将分成的总页数信息通过请求域对象传递给前端
                request.setAttribute("totalPageCount",totalPageCount);
                //传递给前端
                request.setAttribute("currentPageNo",currentPageNo);
